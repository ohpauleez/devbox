...
- [ ] 6.4 Update `openspec/specs/remote-access/spec.md`'s `REMOTE-DOMAIN-PRECOND` Evidence/Alloy model to reflect the new step order (`ensureSshKeyMaterial` now precedes `describeInstance`) — intentionally left unchecked; deferred to archive time per the design's noted risk. This note is the flag so it isn't missed when `openspec archive` runs for this change.
...
- **6.4**: Deliberately not done now — see checkbox note. The base spec's Evidence/Alloy content is populated from real, implemented line numbers (this repo's existing archived changes show Evidence sections citing concrete file:line references), so it belongs at archive time when `openspec archive` reconciles this change's delta into `openspec/specs/remote-access/spec.md`, not while `add-ssh-agent-forwarding` is still an in-progress change.
