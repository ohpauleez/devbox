...
- `connect` requires local `ssh`
- `cp` additionally requires local `scp`
- `connect` and `cp` require local `ssh-keygen` when no suitable SSH public key is available from the local agent for temporary key staging
...
The `connect` and `cp` commands must perform the same end-to-end workflow described within [ssh-over-ssm](https://github.com/elpy1/ssh-over-ssm), including temporary SSH public-key placement on the remote host, bounded cleanup or removal of that temporary key material on the remote host, and session startup over AWS SSM.
...
### Cross-System Partial-Failure Rule

Commands that successfully change AWS state or remote-instance state and then fail to commit the corresponding local config update must fail with `ConsistencyError`.

Rules:

- this applies to `init`, `rm --terminate`, `connect`, and `cp`
- the normalized stderr summary must clearly state that the external operation succeeded but the local config update failed
- the error details should identify which local field or alias may now be stale, such as retained local tracking after accepted termination or a missing `lastConnectAt` update after a successful session or transfer
- these failures must not be downgraded to `ConfigError` because the system state has diverged across local and remote boundaries
...
- temporary SSH key staging for `connect` and `cp` must use explicit subprocess argv, not shell-expanded heredocs or shell-interpolated command strings built from user-controlled values
- temporary remote `authorized_keys` entries created for `connect` or `cp` must be short-lived, scoped to the selected SSH user, and removed automatically or by best-effort cleanup
...
- use the same end-to-end approach as `ssh-over-ssm`, not only the final `ProxyCommand`
- before starting the SSH or SCP session, stage a temporary SSH public key for the target SSH user on the remote instance by using `aws ssm send-command` with `AWS-RunShellScript`
- wait for successful key-staging command completion before starting the SSH transport session
- start the SSH transport session through `aws ssm start-session --document-name AWS-StartSSHSession`
- use the tracked EC2 instance ID as the SSH host token so the SSM session receives the correct target
- use a temporary local keypair when needed, or a suitable existing local SSH public key when available, consistent with the `ssh-over-ssm` workflow
- ensure the temporary remote authorized-key entry is removed automatically after a bounded interval and also attempt best-effort cleanup on local failure paths
- upload to a temp path in the destination directory first
- after `scp` succeeds, invoke `ssh` over the same SSM-backed access path to atomically move the temp file into the requested destination path
- perform best-effort remote cleanup of both the temp file path and any temporary remote SSH key material on failure
...
- config write failure after successful launch leaves the instance running in AWS and reports the divergence clearly to stderr as `ConsistencyError`
...
- with `--terminate`, the instance termination request is submitted first and the alias is removed locally only after AWS confirms termination request acceptance or reports the instance as already absent
...
- with `--terminate`, local config write failure after accepted termination or accepted already-absent handling yields `ConsistencyError`; stderr reports that AWS state changed while local tracking may still remain
...
- caller IAM allows both SSM session startup and the SSM command execution needed to stage temporary SSH access
- instance becomes SSM-ready within the readiness timeout
- local `ssh` executable is installed
- local `ssh-keygen` is installed when no suitable SSH public key is available from the local agent for temporary key staging
- the target SSH user already exists on the remote instance and its home directory can host a temporary `authorized_keys` entry

**Postconditions**
- starts an SSM-backed SSH session to the current instance by performing the same end-to-end operations described within [ssh-ssm.sh](https://raw.githubusercontent.com/elpy1/ssh-over-ssm/refs/heads/master/ssh-ssm.sh), including temporary SSH key placement for the target SSH user before session startup
- removes or schedules removal of the temporary remote SSH key material using bounded best-effort cleanup behavior consistent with `ssh-over-ssm`
- updates `lastConnectAt` only if session startup succeeds and the subsequent local config write succeeds
...
- SSH key staging failure, SSM command failure, SSH startup failure, or cleanup/setup transport failure yields `TransportError`
- local config write failure after successful session startup yields `ConsistencyError`; stderr reports that the session succeeded but `lastConnectAt` may be stale locally
- failed session startup does not update `lastConnectAt`
...
- caller IAM allows both SSM session startup and the SSM command execution needed to stage temporary SSH access
- the target SSH user already exists on the remote instance and its home directory can host a temporary `authorized_keys` entry
- local SSH configuration and credentials can authenticate to the target instance through the staged-key SSM tunnel
...
- the transfer performs the same temporary SSH key placement and bounded cleanup behavior described in `ssh-ssm.sh` before invoking `scp` and `ssh` over SSM
- `lastConnectAt` is updated only after successful transfer completion and the subsequent local config write succeeds
...
- SSH key staging failure, missing local `ssh` or `scp`, SSH authentication failure, SSH host-key failure, `scp` process failure, or remote cleanup/finalization transport failure yields `TransportError`
- remote command failure does not leave a partial file at the final destination path
- local config write failure after successful remote transfer finalization yields `ConsistencyError`; stderr reports that the remote file update succeeded but `lastConnectAt` may be stale locally
...
- `connect` and `cp` must implement the full temporary SSH key staging and cleanup flow from [ssh-ssm.sh](https://raw.githubusercontent.com/elpy1/ssh-over-ssm/refs/heads/master/ssh-ssm.sh), not merely a raw SSM `ProxyCommand`
- temporary SSH key material must be short-lived, scoped to the requested SSH user, and removed automatically or by best-effort cleanup after session setup
...
20. `cp` rejects non-regular files, oversized files, non-running instances, and missing SSH prerequisites
...
9. implement SSM readiness checks, temporary SSH key staging, and `connect`
10. implement `cp` using `scp` and `ssh` over the full `ssh-over-ssm` workflow with temp-path finalization
11. implement `rm --terminate` with documented sequencing and `ConsistencyError` handling
